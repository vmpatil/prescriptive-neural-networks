...
# _Last Updated: September 6th 2022_
...
def B_PT(R, theta, data, covariates, treatment, version, fold):
    ...
    model.setObjective(theta*sum( sum(-nu[(i,n)] for n in T) for i in range(I))
                       - (1-theta)*sum((data['max.sbp'][i] - g[i])**2 for i in range(I)), 
                       GRB.MAXIMIZE) # EC.2a
    ...
    min_avg_outcome = min([data['max.sbp'][i] for i in range(len(data))])
    max_avg_outcome = max([data['max.sbp'][i] for i in range(len(data))])
...
def run_experiment(version):
    ...
    for fold in range(len(fold_idx)-1):
        ...
        obj, mip_gap, mip_runtime, output, rule, assignments = B_PT(ancestor_pathing, 0.5, train_set, covariates, treatment, version, fold)
