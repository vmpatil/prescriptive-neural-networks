...
### Experiments for Prescriptive Trees adapted from Kallus (2017) [Jo et. al]
# 
# _Reference: Jo et. al (2020), Kallus (2017)_

# _Last Updated: 18th August 2022_
...
def K_PT(data, R, num_feats, outcome='y', treatment='t'):
    ...
    M = y_bar_max*max(sum(([1]*I)[i] for i in range(I) if df['t'][i] == k) for k in K)
    ...
    for n in B:
        branching_rule.append([output_dict['b '+str((n,f))] for f in range(1,F+1)])
        ...
        for N in train_lens:
            ...
            for prob in probs:
                ...
                for dataset in datasets:
                    ...
                    obj, mip_gap, mip_runtime, output, rule, assignments = K_PT(df, ancestor_pathing, num_feats, outcome='y', treatment='t')

                    print('\Finished Training '+ datatype + '_' + str(d) + '_' + str(N) + '-' + str(prob) + '_' + str(dataset) + '\n')
