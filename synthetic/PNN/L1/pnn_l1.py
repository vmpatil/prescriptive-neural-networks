...
def PrescriptiveNetwork_L1(data,D,K,λ): 
...
#     filename = 'historic_baseline_'
    ...
    for k in range(K):
        for t in range(T):
            alpha_abs[(k,t,L)] = m.addVar(lb=0, ub=w_ub, vtype=GRB.CONTINUOUS, name="alpha_abs "+str((k,t,L)))
            
        for d in range(D):
            alpha_abs[(d,k,0)] = m.addVar(lb=0, ub=w_ub, vtype=GRB.CONTINUOUS, name="alpha_abs "+str((d,k,0)))
            
        for k_prime in range(K):
            for l in range(1,L):
                alpha_abs[(k_prime,k,l)] = m.addVar(lb=0, ub=w_ub, vtype=GRB.CONTINUOUS, name="alpha_abs "+str((k_prime,k,l)))
    
        for l in range(0,L):
            beta_abs[(k,l)] = m.addVar(lb=0, ub=b_ub, vtype=GRB.CONTINUOUS, name="beta_abs "+str((k,l))) 
    
    for t in range(T):
        beta_abs[(t,L)] = m.addVar(lb=0, ub=b_ub, vtype=GRB.CONTINUOUS, name="beta_abs "+str((t,L)))        
    ...
    m.setObjective(
        (1/N)*sum( sum( (h[(n,t,L)] - baseline[n][t])*(data['psi'+str(t)][n]) for t in range(T)) 
                  for n in range(N)) 
        - λ*(sum( sum(alpha_abs[(d,k,0)] for k in range(K)) for d in range(D)) 
             + sum( sum( sum(alpha_abs[(k_prime,k,l)] for k in range(K)) for k_prime in range(K)) for l in range(1,L))
             + sum( sum(alpha_abs[(k,t,L)] for t in range(T)) for k in range(K))
             + sum( sum(beta_abs[(k,l)] for l in range(0,L)) for k in range(K))
             + sum( beta_abs[(t,L)] for t in range(T))), GRB.MAXIMIZE)
    ...
    for k in range(K):
        for d in range(D):
            m.addConstr(alpha[(d,k,0)] <= alpha_abs[(d,k,0)], name='C1 alpha_abs '+str((d,k,0)))
            m.addConstr(-alpha[(d,k,0)] <= alpha_abs[(d,k,0)], name='C2 alpha_abs '+str((d,k,0)))
            
        for k_prime in range(K):
            for l in range(1,L):
                m.addConstr(alpha[(k_prime,k,l)] <= alpha_abs[(k_prime,k,l)], name='C1 alpha_abs '+str((k_prime,k,l)))
                m.addConstr(-alpha[(k_prime,k,l)] <= alpha_abs[(k_prime,k,l)], name='C2 alpha_abs '+str((k_prime,k,l)))            
        
        for t in range(T):
            m.addConstr(alpha[(k,t,L)] <= alpha_abs[(k,t,L)], name='C1 alpha_abs '+str((k,t,L)))
            m.addConstr(-alpha[(k,t,L)] <= alpha_abs[(k,t,L)], name='C2 alpha_abs '+str((k,t,L)))
            
        for l in range(0,L):
            m.addConstr(beta[(k,l)] <= beta_abs[(k,l)], name='C1 beta_abs '+str((k,l)))
            m.addConstr(-beta[(k,l)] <= beta_abs[(k,l)], name='C2 beta_abs '+str((k,l)))

    for t in range(T):
        m.addConstr(beta[(t,L)] <= beta_abs[(t,L)], name='C1 beta_abs '+str((t,L)))
        m.addConstr(-beta[(t,L)] <= beta_abs[(t,L)], name='C2 beta_abs '+str((t,L)))        
            ...
            for λ in lambda_set:
                ...
                for prob in probs:
                    ...
                    for dataset in datasets:
                        ...
                        obj, mip_gap, mip_runtime, output, params = PrescriptiveNetwork_L1(df,D,K,λ)
