...
def B_PT(R, theta, data, covariates, treatment, seed1, seed2, prob):
    ...
    model.setObjective(theta*sum( sum(nu[(i,n)] for n in T) for i in range(I))
                       - (1-theta)*sum((data['y'].iloc[i] - g[i])**2 for i in range(I)), 
                       GRB.MAXIMIZE) # EC.2a
    ...
    min_avg_outcome = min([data['y'].iloc[i] for i in range(len(data))])
    max_avg_outcome = max([data['y'].iloc[i] for i in range(len(data))])

    M = max_avg_outcome - min_avg_outcome

    for i in range(I):
        for n in T:
            model.addConstr(g[i] - beta[(n,data[treatment].iloc[i])] <= M*(1-lamb[(i,n)]), 
...
def run_experiment(seed1):
    ...
    for seed_dir in os.listdir(os.path.join(dir_path)):
        ...
        for prob in [33,65,85]:
            ...
            obj, mip_gap, mip_runtime, output, rule, assignments = B_PT(ancestor_pathing, 0.5, train, covariates, treatment, seed1, seed2, prob)
